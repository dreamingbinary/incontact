  ...
  class CallListsTest < TestCase
    let(:connection) do
      InContact::Connection.new(TEST_URL, "bearer 1234",
        { default_data_model: InContact::Responses::CallListResponse } )
    end

    let(:call_lists) { InContact::CallLists.new(connection) }
     ...
     describe "#find" do
      ...
      it "will return an CallListResponse" do
        @response.must_be_kind_of(InContact::Responses::CallListResponse)
      end
    ...
    describe "#upload_file" do
      before do
        file_content = File.read("test/incontact/fixtures/valid_call_list.csv")
